...
bams <- file.path("./real_offtargets/simulation_", freqs[w], paste0(base, ".bam"))
...
tr <- data.frame(Truth = levels(result$Truth),
                 TrNum = as.numeric(gsub("%", "", levels(result$Truth))))# danRer7 <- BSgenome.Drerio.UCSC.danRer7
guides <- import("./idx/shah_guides.bed")
guides <- guides + 5
references <- getSeq(danRer7, guides)
...
# data.table::fread("../figures/indel_rate_vs_real_offtargets_0.2.csv")
p <- ggplot(result) +
...
ggplot2::ggsave(paste0("../figures/indel_rate_vs_real_offtargets_", freqs[w], ".png"), p)
