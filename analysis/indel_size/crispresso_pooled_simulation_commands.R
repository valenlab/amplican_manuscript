...
sim_files <- list.files("./indel_size/simulation", pattern = ".*sim1.fq", full.names = TRUE)
...
out_template <- "./indel_size/simulation/merged/%s_%s.fq"
...
amplicons_f <- rep("./indel_size/simulation/merged/crispresso_pooled_amplicons.txt", nr1s)
...
cmds <- paste(sprintf(pooled_template, r1s, r2s, amplicons_f, idx, names(splits)),
          collapse = "\n")
cat(cmds, file = "./indel_size/crispresso_pooled_commands.sh")
cat("\n\nmv CRISPRessoPooled_on_* ./indel_size/simulation/merged/", 
    file = "./indel_size/crispresso_pooled_commands.sh", append = TRUE)
