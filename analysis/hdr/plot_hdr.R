...
for (i in pooled_dirs) {
  ...
  amplican_results <- rbind(amplican_results,
                            data.frame(Guide = this_r$ID,
                                       ...
                                       Indels = noff,
                                       variable = "ampliCan",
                                       value = this_r$HDR / this_r$Reads_Filtered * 100))
}
...
parseCRISPResso <- function(results_dir){
  ...
  result <- sum(counts[2])/counts[4]*100
  c(result, counts[4])
}
...
result <- lapply(seq_along(base), function(i){
  ...
  c(sim_guide, noff[i], truth[i], crispresso[[1]])
})

result <- data.frame(do.call(rbind, result))
colnames(result) <- c("Guide", "Indels", "Truth", "CRISPResso")
result <- melt(result, id.vars=c("Guide", "Truth", "Indels"))
result_save <- result

result <- rbind(result_save, pooled_results, amplican_results)
