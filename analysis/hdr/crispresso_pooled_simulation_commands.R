...
sim_files <- list.files("./hdr/simulation", pattern = ".*sim1.fq", full.names = TRUE)
...
out_template <- "./hdr/simulation/merged/%s_%s.fq"
...
levels <- gsub("freq", "", gsub(".*_([0-9]+freq).*", "\\1", basename(r1s)))
