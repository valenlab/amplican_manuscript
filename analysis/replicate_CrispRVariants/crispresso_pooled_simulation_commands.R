...
sim_files <- list.files("./replicate_CrispRVariants/simulation", pattern = ".*sim1.fq", full.names = TRUE)
...
out_template <- "./replicate_CrispRVariants/simulation/merged/%s_%s.fq"
...
amplicons_f <- rep("./replicate_CrispRVariants/simulation/merged/crispresso_pooled_amplicons.txt", nr1s)
...
cmds <- paste(sprintf(pooled_template, r1s, r2s, amplicons_f, idx, names(splits)),
          collapse = "\n")
cat(cmds, file = "./replicate_CrispRVariants/crispresso_pooled_commands.sh")
cat("\n\nmv CRISPRessoPooled_on_* ./replicate_CrispRVariants/simulation/merged/", file = "./replicate_CrispRVariants/crispresso_pooled_commands.sh", append = TRUE)
