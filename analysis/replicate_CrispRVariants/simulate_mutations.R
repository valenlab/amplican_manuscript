...
guides <- rtracklayer::import("./idx/shah_guides.bed")
guides <- guides[match(amplicons$name, guides$name)]
guides <- guides + 5
adiv_out <- "./replicate_CrispRVariants/amplicondivider_simulation_commands.sh"
...
sample_seqs <- function(n_mut, n_original, n_offtarget, amplicons,
  ...
  for (i in 1:nrow(amplicons)) {
    ...
    adiv_dir <- "../../analysis/replicate_CrispRVariants/simulation/amplicondivider"
    ...
    a1 <- sprintf(adiv_tmp1, file.path("../../analysis/replicate_CrispRVariants/simulation", basename(bam)), gd_rng)
...
sim_cmds <- "./replicate_CrispRVariants/simulation_commands.sh"
crispresso_cmds <- "./replicate_CrispRVariants/crispresso_simulation_commands.sh"

for (nofftargets in c(100,33,0)){
  # cntn3b_0mut_300wt_100offtarget_200readlen.fa
    # 0% efficient
    sample_seqs(0,300, nofftargets, amplicons, "./replicate_CrispRVariants/simulation",
               sim_file = sim_cmds, crispresso_file = crispresso_cmds)

    # 33% efficient
    sample_seqs(100,200, nofftargets, amplicons, "./replicate_CrispRVariants/simulation",
               sim_file = sim_cmds, crispresso_file = crispresso_cmds)

    # 66% efficient
    sample_seqs(200,100, nofftargets, amplicons, "./replicate_CrispRVariants/simulation",
               sim_file = sim_cmds, crispresso_file = crispresso_cmds)

    # 90% efficient
    sample_seqs(270,30, nofftargets, amplicons, "./replicate_CrispRVariants/simulation",
               sim_file = sim_cmds, crispresso_file = crispresso_cmds)
}
...
cat(amplicon_inf, file = "./replicate_CrispRVariants/simulation/merged/crispresso_pooled_amplicons.txt")
cat("\n\nmv crispresso/* ./replicate_CrispRVariants/simulation/crispresso; rmdir crispresso\n",
